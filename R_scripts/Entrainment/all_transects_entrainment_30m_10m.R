...
#### Each bin is a potential tidal turbine ####

setwd("~/UW Summer 2022/EV Exports/Mobile/Entrainment/Horizontal_Axis")
...
all_ent_horizontal <- load_data("~/UW Summer 2022/EV Exports/Mobile/Entrainment/Horizontal_Axis")

#### Obtaining abundances ####

for (i in 1:length(all_ent_horizontal$PRC_ABC)) {
  all_ent_horizontal$abundance[i] <- NA
  all_ent_horizontal$abundance[i] <- all_ent_horizontal$PRC_ABC[i]*area/sigma_bs_herring
}

#### Creating new data frames for filtering ####

each_ent_horizontal <- all_ent_horizontal %>%
  group_by(Region_name, Dist_S) %>%
  dplyr::summarise(Sum_each = sum(abundance))

domain_ent_horizontal <- all_ent_horizontal %>%
  group_by(Region_name) %>%
  dplyr::summarise(Sum_region = sum(abundance))

#### Combine data frames and division function ####

all_transects <- merge(each_ent_horizontal, domain_ent_horizontal, by = "Region_name")
...
day_ent_horizontal <- load_data("~/UW Summer 2022/EV Exports/Mobile/Entrainment/Horizontal_Axis")

#### Filter transects ####

day_ent_horizontal <- day_ent_horizontal %>% filter(Region_name != "GRID_B_N_NIGHT_6_04_12" &
...
for (i in 1:length(day_ent_horizontal$PRC_ABC)) {
  day_ent_horizontal$abundance[i] <- NA
  day_ent_horizontal$abundance[i] <- day_ent_horizontal$PRC_ABC[i]*area/sigma_bs_herring
}

#### Creating new data frames for filtering ####

day_each_ent_horizontal <- day_ent_horizontal %>%
  group_by(Region_name, Dist_S) %>%
  dplyr::summarise(Sum_each = sum(abundance))

day_domain_ent_horizontal <- day_ent_horizontal %>%
  group_by(Region_name) %>%
  dplyr::summarise(Sum_region = sum(abundance))

#### Combine data frames and division function ####

day_all_transects <- merge(day_each_ent_horizontal, day_domain_ent_horizontal, by = "Region_name")
...
night_ent_horizontal <- load_data("~/UW Summer 2022/EV Exports/Mobile/Entrainment/Horizontal_Axis")

#### Filter transects ####

night_ent_horizontal <- night_ent_horizontal %>% filter(Region_name %in% c("GRID_B_N_NIGHT_6_04_12", 
...
for (i in 1:length(night_ent_horizontal$PRC_ABC)) {
  night_ent_horizontal$abundance[i] <- NA
  night_ent_horizontal$abundance[i] <- night_ent_horizontal$PRC_ABC[i]*area/sigma_bs_herring
}

#### Creating new data frames for filtering ####

night_each_ent_horizontal <- night_ent_horizontal %>%
  group_by(Region_name, Dist_S) %>%
  dplyr::summarise(Sum_each = sum(abundance))

night_domain_ent_horizontal <- night_ent_horizontal %>%
  group_by(Region_name) %>%
  dplyr::summarise(Sum_region = sum(abundance))

#### Combine data frames and division function ####

night_all_transects <- merge(night_each_ent_horizontal, night_domain_ent_horizontal, by = "Region_name")
...
#### Sub-setting an arbitrary ZOI per transect ####

## Day & Night

all_list <- as.list(split(all_transects, f = all_transects$Region_name))

all_list_probs <- vector()

for (i in 1:length(all_list)) {
  all_list_probs[i] <- all_list[[i]][10,]$Sum_each/all_list[[i]][10,]$Sum_region
}

hist(all_list_probs, breaks = 40, xlab = "Probability", main = "Day & Night Probabilities")
all_mean <- mean(all_list_probs)
abline(v = all_mean, col = "red")

## Day

day_list <- as.list(split(day_all_transects, f = day_all_transects$Region_name))

day_list_probs <- vector()

for (i in 1:length(day_list)) {
  day_list_probs[i] <- day_list[[i]][10,]$Sum_each/day_list[[i]][10,]$Sum_region
}

hist(day_list_probs, breaks = 40, xlab = "Probability", main = "Day Probabilities")
day_mean <- mean(day_list_probs)
abline(v = day_mean, col = "red")

## Night

night_list <- as.list(split(night_all_transects, f = night_all_transects$Region_name))

night_list_probs <- vector()

for (i in 1:length(night_list)) {
  night_list_probs[i] <- night_list[[i]][10,]$Sum_each/night_list[[i]][10,]$Sum_region
}

hist(night_list_probs, breaks = 20, xlab = "Probability", main = "Night Probabilities")
night_mean <- mean(night_list_probs)
abline(v = night_mean, col = "red")
