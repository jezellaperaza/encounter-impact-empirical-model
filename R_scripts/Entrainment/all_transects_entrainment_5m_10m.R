...
setwd("~/UW Summer 2022/EV Exports/Mobile/Entrainment/Axial_Flow")
...
all_ent_axial <- load_data("~/UW Summer 2022/EV Exports/Mobile/Entrainment/Axial_Flow")

#### Obtaining abundances ####

for (i in 1:length(all_ent_axial$PRC_ABC)) {
  all_ent_axial$abundance[i] <- NA
  all_ent_axial$abundance[i] <- all_ent_axial$PRC_ABC[i]*area/sigma_bs_herring
}

#### Creating new data frames for filtering ####

each_ent_axial <- all_ent_axial %>%
  group_by(Region_name, Dist_S) %>%
  dplyr::summarise(Sum_each = sum(abundance))

domain_ent_axial <- all_ent_axial %>%
  group_by(Region_name) %>%
  dplyr::summarise(Sum_region = sum(abundance))

#### Combine data frames and division function ####

all_transects <- merge(each_ent_axial, domain_ent_axial, by = "Region_name")
...
day_ent_axial <- load_data("~/UW Summer 2022/EV Exports/Mobile/Entrainment/Axial_Flow")

#### Filter transects ####

day_ent_axial <- day_ent_axial %>% filter(Region_name != "GRID_B_N_NIGHT_6_04_12" &
...
for (i in 1:length(day_ent_axial$PRC_ABC)) {
  day_ent_axial$abundance[i] <- NA
  day_ent_axial$abundance[i] <- day_ent_axial$PRC_ABC[i]*area/sigma_bs_herring
}

#### Creating new data frames for filtering ####

day_each_ent_axial <- day_ent_axial %>%
  group_by(Region_name, Dist_S) %>%
  dplyr::summarise(Sum_each = sum(abundance))

day_domain_ent_axial <- day_ent_axial %>%
  group_by(Region_name) %>%
  dplyr::summarise(Sum_region = sum(abundance))

#### Combine data frames and division function ####

day_all_transects <- merge(day_each_ent_axial, day_domain_ent_axial, by = "Region_name")
...
night_ent_axial <- load_data("~/UW Summer 2022/EV Exports/Mobile/Entrainment/Axial_Flow")

#### Filter transects ####

night_ent_axial <- night_ent_axial %>% filter(Region_name %in% c("GRID_B_N_NIGHT_6_04_12", 
...
for (i in 1:length(night_ent_axial$PRC_ABC)) {
  night_ent_axial$abundance[i] <- NA
  night_ent_axial$abundance[i] <- night_ent_axial$PRC_ABC[i]*area/sigma_bs_herring
}

#### Creating new data frames for filtering ####

night_each_ent_axial <- night_ent_axial %>%
  group_by(Region_name, Dist_S) %>%
  dplyr::summarise(Sum_each = sum(abundance))

night_domain_ent_axial <- night_ent_axial %>%
  group_by(Region_name) %>%
  dplyr::summarise(Sum_region = sum(abundance))

#### Combine data frames and division function ####

night_all_transects <- merge(night_each_ent_axial, night_domain_ent_axial, by = "Region_name")
...
hist(all_list_probs, breaks = 30, xlab = "Probability", main = "Day & Night Probabilities")
...
hist(day_list_probs, breaks = 30, xlab = "Probability", main = "Day Probabilities")
