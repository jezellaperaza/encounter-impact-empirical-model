...
for (i in 1:length(all_list)) {
  all_list_probs[i] <- all_list[[i]][5,]$Sum_each/all_list[[i]][5,]$Sum_region
}

hist(all_list_probs, breaks = 20, xlab = "Probability", main = "Day & Night Probabilities")
...
for (i in 1:length(day_list)) {
  day_list_probs[i] <- day_list[[i]][5,]$Sum_each/day_list[[i]][5,]$Sum_region
}

hist(day_list_probs, breaks = 20, xlab = "Probability", main = "Day Probabilities")
...
for (i in 1:length(night_list)) {
  night_list_probs[i] <- night_list[[i]][5,]$Sum_each/night_list[[i]][5,]$Sum_region
}

hist(night_list_probs, breaks = 10, xlab = "Probability", main = "Night Probabilities")
night_mean <- mean(night_list_probs)
abline(v = night_mean, col = "red")

#### Justification for the -70 dB threshold? ####

mean(all_transects$Sum_each)
25*sigma_bs_herring
10*log10(0.001847605)
TS_herring + -27.33391
